...
def clean_term(term):
    """Convert a term to a glossary-friendly format."""
    # Remove any special characters and convert to lowercase
    term = re.sub(r'[^\w\s-]', '', term)
    # Replace spaces with hyphens and convert to lowercase
    return term.lower().replace(' ', '-')

def extract_lexicon_links(file_path):
    with open(file_path, 'r', encoding='utf-8') as f:
        content = f.read()
    
    # Find all LEXICON links and their context
...
def main():
    ...
    for root, _, files in os.walk(docs_dir):
        for file in files:
            if file.endswith('.md'):
                ...
                mappings = extract_lexicon_links(file_path)
    ...
    for page_id, terms in sorted(all_mappings.items()):
        ...
        glossary_term = clean_term(terms_list[0][1])  # Use the first path we found
